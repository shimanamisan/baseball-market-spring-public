...
### バージョンタグの付与タイミング（開発フェーズ中は保留）

**現在はリプレース開発フェーズのため、`vX.Y.Z` タグの付与は不要**とする。`develop` → `main` のマージは行うが、タグは打たない。

- タグ付与を開始するのは **ステージング環境へのデプロイ時点から**。そのタイミングはユーザーが明示的に通知する
- ユーザーからの通知があるまでは、上記「リリース手順」のステップ 4（タグ付与）はスキップしてよい
- 通知後は本ルール本来の手順（`vX.Y.Z` タグ付与 + push）に復帰する
...
- リリースは `develop` → `main` の直接マージ（PR のタイトル・説明は空欄にしない）。`vX.Y.Z` タグ付与は**開発フェーズ中は保留**（「バージョンタグの付与タイミング」を参照。ステージングデプロイ時点からユーザー通知を受けて開始）
- ホットフィックス時は `main` と `develop` の両方へマージする（タグ付与は上記タイミング方針に従う）
